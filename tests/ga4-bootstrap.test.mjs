...
test('GA4 bootstrap exposes no custom events or restricted assessment payload data', () => {
  ...
  assert.doesNotMatch(
    ga4Source,
    /assessment_start|assessment_complete|screen_view|feedback_click|language_change|theme_change|answer_selection|rank_assignment|personality_result|animal_result/,
  );
  assert.doesNotMatch(
    ga4Source,
    /questionId|optionId|selectedOptionId|rankings|AssessmentAnswer|AssessmentResult|DimensionProfile|PoleScoreMap|adaptiveQuestion|adaptiveRoute|lockedPrimary|personalProfile|professionalProfile|contextProfile|personalityType|primaryTypeId|secondaryTypeId|balancedDimension|primaryAnimal|secondaryAnimal|animalId|matchPercentage|distance|confidence|modelScore|buildVersion|feedbackRecipient/,
  );
  assert.doesNotMatch(ga4Source, /features\/(?:assessment|personalities|animals|results)/);
